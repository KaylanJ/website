  ...
  try {
    ...
    if (file_name && file_content) {
      ...
      if (language === 'python') {
        ...
        finalScript = init + script.replace(/int\s+main\s*\(\s*\)\s*\{/, 'int main() { __init();');
      }
        ...
        finalScript = script.replace(/public\s+static\s+void\s+main\s*\(\s*String\s+args\s*\[\s*\]\s*\)\s*\{/, jInject);
      }
    }
    ...
    return NextResponse.json({ output: "SERVER_ERROR" }, { status: 500 });
  }
}